...
uniform sampler2D textureGlossy;
...
void main()
{
    ...
    vec4 textureGlossyColor = texture( textureGlossy, vertexUV);
    if( onOff == 0){
        color = (specular + diffuse + ambient) * textureColor.xyz * textureGlossyColor.xyz *object_color;
    }else{
        color =  ambient * 2 * textureColor.xyz * textureGlossyColor.xyz *object_color;
    }
